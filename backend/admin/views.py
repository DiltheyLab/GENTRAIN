...
class PanelView(AdminIndexView):
    def is_visible(self):
        return False
...
class PathogenView(AuthModelView):
    ...
    def after_model_change(self, form, model, is_created):
        ...
        if is_created is False and model.scheme_name and model.scheme_name != self.prior_scheme_name:
            rename(path.join(get_project_path(), f"schemes/{self.prior_scheme_name}"),
                   path.join(get_project_path(), f"schemes/{model.scheme_name}"))
