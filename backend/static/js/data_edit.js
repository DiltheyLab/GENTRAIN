...
function register_events_sequences_fasta() {
    ...
    d3.select("#seq_ims_start").on("click", function () {
        ...
        if (!sequence_fasta_check_files("", "", "seq_ims_tsv_file_chooser", "seq_ims_tsv_error_message")) {
            sequence_fasta_after(true, "");
            return;
        }

        //  unhide the progressbar and alerts
        hide_reveal_by_ids(["seq_ims_hidden_bar", "seq_ims_hidden_messages"], false);
        ...
        try {
            ...
            tsv_reader.onload = function (evt) {
                ...
                collect_data_add_to_DB(tsv_array, [], [], {}, "#seq_ims_progressbar", "seq_ims_messages")
    ...
    d3.select("#seq_fasta_start").on("click", function () {
        ...
        if (!sequence_fasta_check_files("seq_fasta_file_chooser", "seq_fasta_error_message", "seq_fasta_tsv_file_chooser", "seq_fasta_tsv_error_message")) {
            sequence_fasta_after(true, "");
            return;
        }

        //  unhide the progressbar and alerts
        hide_reveal_by_ids(["seq_fasta_hidden_bar", "seq_fasta_hidden_messages"], false);
        ...
        try {
            ...
            tsv_reader.onload = function (evt) {
                ...
                if (document.getElementById("seq_fasta_file_chooser").files.length == 1) {
                    ...
                    fa_reader.onload = function (evt) {
                        ...
                        if (warnings.length > 0) {
                            ...
                            sequence_fasta_after(false, "Stopped process: Syntax error in fasta");
                            return;
                        }
                        ...
                        for (var i in needed_fastaids) {
                            if (!fasta_ids.includes(needed_fastaids[i])) {
                                sequence_fasta_after(false, `Stopped process: fasta id ${needed_fastaids[i]} from tsv file not found in fasta file.`);
                                return;
                            }
                        }

                        // report syntax check success
                        add_alert("seq_fasta_messages", "success", "Dateien fertig eingelesen.");
                        ...
                        collect_data_add_to_DB(tsv_array, fasta_array, fasta_ids, fasta_id_to_sequence, "#seq_fasta_progressbar", "seq_fasta_messages")
                    ...
                    if (needed_fastaids.length > 0) {
                        sequence_fasta_after(false, `Prozess abgebrochen: Fasta id ${needed_fastaids[0]} aus tsv Datei wurde nicht in der fasta Datei gefunden.`);
                        // sequence_fasta_after(false, `Stopped process: fasta id ${needed_fastaids[0]} from tsv file not found in fasta file.`);
                        return;
                    }
                    // continue process ( in new function and not after else because file reading is async )
                    collect_data_add_to_DB(tsv_array, [], [], {}, "#seq_fasta_progressbar", "seq_fasta_messages")
...
// Output: -
function collect_data_add_to_DB(tsv_array, fasta_array, fasta_ids, fasta_id_to_sequence, progressbar_id, alertbox_id) {
    ...
    for (var i in tsv_array) {
        ...
        if (tsv_array[i][0] == "a") {
            ...
            sequence_fasta_after(false, `Prozess abgebrochen: Option ${fastaid_to_alldata[fasta_id]["edit_option"]} ist nicht erlaubt.`);
    ...
    // ####################################
    // report status
    add_alert(alertbox_id, "info", "Sequenzen und metadaten der IMS-IDs werden heruntergeladen.");
    ...
    var request = d3.json("/data/IMS_to_fasta", {
        method: "POST",
        ...
        if (imsid_to_data == null) {
            sequence_fasta_after(false, `Stopped process: An error occured while getting the information for the IMS-IDS`);
            return;
        }
        ...
        for (var fasta_id in fastaid_to_alldata) {
            ...
            if (ims_id in imsid_to_data) {
                fastaid_to_alldata[fasta_id]["sequence"] = imsid_to_data[ims_id]["sequence"];
                fastaid_to_alldata[fasta_id]["sampling_date"] = imsid_to_data[ims_id]["sampling_date"];
                fastaid_to_alldata[fasta_id]["location_sending_lab"] = imsid_to_data[ims_id]["sending_lab_city"] + ", " + imsid_to_data[ims_id]["sending_lab_zipcode"];
                fastaid_to_alldata[fasta_id]["location_sequencing_lab"] = imsid_to_data[ims_id]["sequencing_lab_city"] + ", " + imsid_to_data[ims_id]["sequencing_lab_zipcode"];
        ...
        add_alert(alertbox_id, "info", "Varianten und Lineage werden mit Nextclade berechnet.");
        ...
        for (var fasta_id in fastaid_to_alldata) {
            ...
            fasta_content_nextclade += ">" + pseudo_count + "\n" + fastaid_to_alldata[fasta_id]["sequence"] + "\n"
        ...
        if (nextclade_data == null) {
            sequence_fasta_after(false, `Prozess abgebrochen: Es gab ein Fehler mit Nextclade.`);
        ...
        for (var i in nextclade_data["results"]) {
            ...
            fastaid_to_alldata[fasta_id]["lineage"] = current_result["clade"] + ", " + current_result["customNodeAttributes"]["Nextclade_pango"]
            fastaid_to_alldata[fasta_id]["N_count"] = current_result["totalMissing"] // + current_result["totalNonACGTNs"] // TODO this ok?
        ...
        add_alert(alertbox_id, "info", "Fälle werden in das lokale Datenset integriert.");
        ...
        return calculate_updated_distancematrix(fasta_ids_toadd, sequences_remove);
        ...
        if (success) {
            ...
            // dm failed
            sequence_fasta_after(false, "Es sind Fehler beim berechnen der Distanzmatrix aufgetreten. Fälle wurden dennoch hinzugefügt.");
        ...
        console.log(e);
        sequence_fasta_after(false, `Prozess abgebrochen! Folgender Fehler ist aufgetreten: ${e}`);
...
function check_tsv_content(tsv_array) {
    ...
    for (var i = 0; i < tsv_array.length; i++) {
        ...
        if (line.length != correct_line_length) {
            add_alert("seq_fasta_messages", "warning", `Zeile ${i + 1} hat eine falscha Anzahl von Spalten. Es sollten ${correct_line_length} sein, aber es gibt ${line.length} Spalten.`);
        ...
        if (!["a", "d"].includes(line[0])) {
            add_alert("seq_fasta_messages", "warning", `Zeile ${i + 1} hat eine falsche Aktion. Es sollte a oder d sein, aber es ist ${line[0]}.`);
        ...
        if (line[2] != "" && !is_valid_IMSID(line[2])) {
            add_alert("seq_fasta_messages", "warning", `Zeile ${i + 1} hat eine falsche IMS-ID. Eine IMD-ID sollte so aussehen: IMS-XXXXX-CVDP-XXXXXXXX-XXXX-XXXX-XXXXXXXXXXXX . Gefundene ID: ${line[2]}.`);
        ...
        if (line[4] != "" && isNaN(Date.parse(line[4]))) {
            add_alert("seq_fasta_messages", "warning", `Zeile ${i + 1} hat ein falsches Datum. Es sollte dem ISO 8601 format folgen (zum Beispiel YYYY-MM-DD), aber es ist ${line[4]}.`);
...
function sequence_fasta_check_files(fa_chooser, fa_error, tsv_chooser, tsv_error) {
    ...
    else if ((fa_chooser != "") && (fa_files.length == 1)) { // somwhat wierd. rewrite this
        if (!(fa_files[0].name.endsWith(".fasta") || fa_files[0].name.endsWith(".fa"))) {
            d3.select("#" + fa_error).html("Falsch Dateiendung. Sollte '.fasta' oder '.fa' sein.")
...
function sequence_fasta_before() {
    ...
    clean_messages_by_ids(["seq_fasta_error_message", "seq_fasta_tsv_error_message", "seq_ims_tsv_error_message"]);

    // hide the progressbar and alerts and reset them
    hide_reveal_by_ids(["seq_fasta_hidden_bar", "seq_fasta_hidden_messages"], true);
