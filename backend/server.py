...
# @app.route("pathogens/covid-19/sequences/V1234/variants", methods=["POST"])
# @app.route("pathogens/test-1/sequences/Z1234/variants", methods=["POST"])
...
@app.route(
    "/pathogens/<pathogen_identifier>/sequences/<fasta_id>/variants", methods=["POST"]
)
def nextclade(pathogen_identifier, fasta_id):
    ...
    process = subprocess.run([f"./scripts/{pathogen_identifier}.sh", fa_tmp, json_tmp])
