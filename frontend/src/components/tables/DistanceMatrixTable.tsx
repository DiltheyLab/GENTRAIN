        ...
        if (distanceMatrixAssembly) {
            return (
                <TableRow key={rowKey}>
                    ...
                    {Object.keys(distanceMatrixAssembly).map((colKey, colIndex) => {
                        return (
                            <TableCell
                                key={colKey}
                                className={`[&:not(:last-child)]:border-r-[1px] border-muted font-medium p-2 text-center text-xs hover:font-bold ${
                                    hoveredRow === rowIndex || hoveredColumn === colIndex ? "bg-muted" : "bg-muted/20"
                                }`}
                                onMouseEnter={() => {
                        ...
                        <Table>
                            <TableBody>
                                <TableRow className="bg-muted/30 border-muted p-2">
                                    ...
                                    {Object.keys(distanceMatrixAssembly).map((key, index) => (
                                        <TableCell
                                            key={key}
                                ...
                                {Object.keys(distanceMatrixAssembly).map((key, index) => renderRow(key, index))}
                            </TableBody>
                        </Table>
                    </div>
                </>
