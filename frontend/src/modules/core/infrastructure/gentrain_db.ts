...
gentrainDB.version(1.2).stores({
    sequence_analyses: "++id, sequence_hash, result, schema, version, pathogen_id, created_at, updated_at",
...
gentrainDB.on("populate", async () => {
    ...
    for (const pathogen of pathogens) {
        ...
        if (!(await gentrainDB.pathogens.get(pathogen.id))) {
            gentrainDB.pathogens.add({
                id: pathogen.id,
                name: pathogen.name,
...
gentrainDB.tables.forEach(function (table) {
